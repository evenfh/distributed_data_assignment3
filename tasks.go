...
func task1(activityService *activity.Service, userService *user.Service, trackpointService *trackpoint.Service) error {
	...
	table.Append([]string{
		"Activity", strconv.Itoa(activityCount),
	})
	table.Append([]string{
...
func task8(userService *user.Service) error {
	...
	for _, u := range users {
		table.Append([]string{
			u.UserID,
			strconv.Itoa(int(u.GainedAltitude)) + "m",
		})
	}
	table.Render()
	return nil
...
func task11(activityService *activity.Service) error {
	users, counts, err := activityService.GetTopTransportationByUsers()
	if err != nil {
		return err
	}
	table := tablewriter.NewWriter(os.Stdout)
	...
	for i, u := range users {
		a := counts[i]
		table.Append([]string{u, a.Mode, strconv.Itoa(int(a.Count))})
	}
